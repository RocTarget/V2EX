...
public enum Type {
    ...
    case notification(seat: Seat, title: String?)
}
...
open class Flea: UIView {
    ...
    open var anchorsite = Anchor.center(nil)
    open var style = FleaStyle.normal(.white)
    open var backgroundStyle = FleaBackgroundStyle.clear
    open var tapDismiss: Bool = true
    ...
    public init(type: Type) {
        ...
        case .notification(let seat, let title):
            anchorsite = .edge(.top)
            backgroundStyle = .none
            style = .blur(.dark)
            contentView = FleaNotificationView()
            (contentView as? FleaNotificationView)?.seat = seat
            (contentView as? FleaNotificationView)?.title = title
            (contentView as? FleaNotificationView)?.flea = self
        }
    }

    convenience init(type: Type, anchor: Anchor, backgroundStyle: FleaBackgroundStyle = .dark, cornerRadius: CGFloat = 0 ) {
        self.init(type: type)
        anchorsite = anchor
    ...
    func prepared() {
        ...
        func layoutInitialPosition(direction: Direction) {
            switch direction {
            case .top:
                initialPosition = CGPoint(x: (bounds.width - containerView.bounds.width) / 2 + finalOffset.horizontal,
        ...
        case .center(let direction):
            finalPosition = CGPoint(x: bounds.midX - containerView.bounds.width/2 + offset.horizontal,
                                    y: bounds.midY - containerView.bounds.height/2 + finalOffset.vertical)
...
extension Flea {
    ...
    fileprivate func show(inView view: UIView) {
        ...
        func animations() {
            if anchorsite.isCenter && anchorsite.direction == nil {
        ...
        if spring {
            UIView.animate(
                withDuration: animationSpringDuration,
                delay: 0,
        ...
        if duration > 0 {
            DispatchQueue.main.asyncAfter(
                deadline: DispatchTime.now() + Double(Int64(Double(NSEC_PER_SEC) * duration)) / Double(NSEC_PER_SEC),
                execute: {
                    self.dismiss()
            })
        }
...
extension Flea {
    public func addAction(_ title: String, color: UIColor = .black, action: (() -> Void)?) {
    ...
    public var subTitleAttributedString: NSAttributedString? {
        get {
            switch type {
            case .actionSheet:
                return (contentView as? FleaActionView)?.subTitleLabel.attributedText
            case .alert:
                return (contentView as? FleaAlertView)?.subTitleLabel.attributedText
            default:
                return nil
            }
        }
        set {
            switch type {
            case .actionSheet:
                (contentView as? FleaActionView)?.subTitleLabel.attributedText = newValue
            case .alert:
                (contentView as? FleaAlertView)?.subTitleLabel.attributedText = newValue
            default:
                break
            }
        }
